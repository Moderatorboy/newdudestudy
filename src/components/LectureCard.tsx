...
import type { Lecture } from '../data/types';  // ✅ use types from your new structure

type Props = {
  lecture: Lecture & { image?: string }; // ✅ allow image fallback
...
export default function LectureCard({ lecture, onToggleComplete }: Props) {
  ...
  return (
    <div className="flex items-center gap-4 p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
      {/* ✅ use lecture.image (chapter photo fallback) */}
      <img
        src={lecture.image}
        alt={lecture.title}
